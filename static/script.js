...
function clearChat() {
    const chatBox = document.getElementById('chat-box');
    chatBox.innerHTML = '<div class="empty-state"><h2>OG\'s here..</h2><p>How can I help you today?</p></div>';
...
function sendMessage() {
    ...
    chatBox.innerHTML += `<div class="message bot"><div class="message-content typing"></div></div>`;
    chatBox.scrollTop = chatBox.scrollHeight;
    ...
    .then(data => {
     ...
     //   lastMessage.textContent = data.response || data.error;  // here we are change the field
     lastMessage.innerHTML = marked.parse(data.response);
